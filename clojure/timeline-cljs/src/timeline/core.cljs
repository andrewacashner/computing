...
;(defn read-events
;  [filename]
;  (let [input (edn/read-string (slurp filename))]
;    (map map->Event input)))
...
(defn event-string-clue
  ...
  [event]
  (gstring/format "(%s) %s" (:place event) (:description event)))

(defn event-string-indexed
  "Format string for an event as part of an ordered timeline"
  [index event]
  (gstring/format "%d. %s (%s) %s" (inc index) (:date event) (:place event) (:description event)))
...
(defn status-game-over
  ...
  (str/join "\n"
           (list  "\nGame over"
                 (gstring/format "Points: %d" points)
                 "\nFinal timeline:"
                  (timeline-string timeline))))
  ...
  (if (empty? clues)
    ...
    (let [clue (first clues)]
      (do
        ...
        (let [index (dec (gstring/parseInt (read-line)))]
          (if (timeline-insert-valid? timeline clue index)
            (do
              (println (gstring/format "Correct! Points now: %d" (inc points)))
              (quiz (rest clues) 
                    (list-insert-index timeline clue index)
                    (inc points)))
            (do
              (println (gstring/format "Incorrect! Points now: %d" (dec points)))
...
(def Timeline (list
 ...
 {:date "1918-11-11"
  :place "Global"
  :description "World War I ends with armistice"
  }
 {:date "1066"
  :place "Hastings England"
  :description "William the Conqueror leads the Normans to victory in England"
  }
 {:date "1918"
  :place "Global"
  :description "Spanish Flu epidemic"
  }))
...
(defn -main
  ...
  (let ;[infile (if (empty? args) default-infile (first args))
       ...
       [timeline (map map->Event Timeline)]
    (play timeline)))
